...
const serviceFields: (keyof ServiceInput)[] = [
  'id',
  'title',
  'description',
  'image',
...
function toPublicService(row: ServiceRowInput & { created_at?: string; updated_at?: string }) {
  return {
    id: row.id,
    title: row.title,
...
function normalizeService(input: Partial<ServiceInput>, partial = false): Partial<ServiceRowInput> {
  const picked = pick<ServiceInput>(input as Record<string, unknown>, serviceFields);
  ...
  try {
    ...
    if (resource === 'services') {
      if (req.method === 'GET') {
        ...
        sendJson(res, 200, { services: (data || []).map(toPublicService), total: count || 0, page, pageSize });
        return;
      }
